...
import {Phase0QualitiesPart0, TestInfoInterface} from "@/components/Test/Properties";
...
const Phase0Part0Component: React.FC<Props> = ({handleContinueButton, testInfo}) => {
    ...
    const updateTestInfo = () => {
        eliminatedQualities.forEach((val, index) => {
            if(!val)
                testInfo.phase0.group0.push(Phase0QualitiesPart0[index].text)
        });
    }
    ...
    return (
        <>
            ...
            <div>
                ...
                {Phase0QualitiesPart0.map((item, i) =>
